...
def load_prefix_info(biolink_version: str) -> Dict[str, Dict[str, str]]:
    ...
    vocab_info_map = {cleaners.clean_vocab_prefix(prefix): {'prefix': prefix, 'iri': iri}
...
def load_validator_map() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        'lm': {'validator': validators.is_lipidmaps_id, 'cleaner': lambda x: x.removeprefix('LM'), 'aliases': ['lipidmaps']},
        ...
        'slm': {'validator': validators.is_slm_id, 'cleaner': lambda x: x.removeprefix('SLM:'), 'aliases': ['swisslipids']},
